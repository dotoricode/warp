...
use crate::ai::blocklist::{
    ...
    InputTypeAutoDetectionSource, PendingAttachment, PendingFile, QueuedQuery, QueuedQueryId,
    QueuedQueryModel, QueuedQueryOrigin, SlashCommandRequest, BLOCK_CONTEXT_ATTACHMENT_REGEX,
...
impl Input {
    ...
    fn handle_queued_prompts_panel_event(
        ...
        match event {
            ...
            QueuedPromptsPanelEvent::RowDeleted { text, is_command } => {
                if self.buffer_text(ctx).is_empty() {
                    self.replace_buffer_content(text, ctx);
                    if *is_command {
                        self.set_input_mode_terminal(/* steal_focus */ false, ctx);
                    }
                }
                self.focus_input_box(ctx);
            }
